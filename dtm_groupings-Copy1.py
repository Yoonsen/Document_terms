...
def countby(dedup=None, counts=None, column=None):
    ...
    :param counts: an instance of Counts
    :param column: the column to aggregate over"""
    
    cols = list(counts.counts.columns)
    result = pd.merge(
        dedup[['urn', column]], 
        counts.counts.transpose().reset_index(), 
        left_on='urn', 
...
if corpus_defined:
    ...
    with st.form("my_form"):
        ...
        with colA:
            st.selectbox('Velg grupperingskolonne', options = ["--ingen gruppering--"] + list(df.columns()))
            transpose = tr
